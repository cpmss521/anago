...
class Vocabulary(object):
    ...
    def __init__(self, max_size=None, lower=True, unk_token=True, specials=('<pad>',)):
        ...
        self._lower = lower
        self._unk = unk_token
        self._token2id = {token: i for i, token in enumerate(specials)}#词典{单词:索引}
        self._id2token = list(specials)#列表
        self._token_count = Counter()#统计最常出现的单词
    ...
    def add_documents(self, docs):
        ...
        Args:
            docs (list): documents to add. 所有的句子
        """
        for sent in docs:
            sent = map(self.process_token, sent)
            self._token_count.update(sent)#会每一个句子的单词的索引基座句子的id

    def doc2id(self, doc):
        """Get the list of token_id given doc.

        Args:
            doc (list): document.doc表示一个分词的句子列表

        Returns:
            list: int id of doc.
        """
        doc = map(self.process_token, doc)#将每个token转化为小写
    ...
    def build(self):
        ...
        Build vocabulary.
        """
        token_freq = self._token_count.most_common(self._max_size)# is None, then list all element counts.
        idx = len(self.vocab)
